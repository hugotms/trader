...
| `EXCHANGE_API_KEY`        | Your One Trading API token used to connect to the API (required when `EXCHANGE_TYPE` is `ONETRADING`)                                                                                                                                                           | no       | None    |
| `EXCHANGE_INPUT_FILENAME` | The filename of the CSV file placed in `./input/` directory (required when `EXCHANGE_TYPE` is `HISTORY`)                                                                                                                                                        | no       | None    |
| `BASE_FIAT`               | Defines the fiat currency that is the base of crypto currencies managed by the bot. If `EXCHANGE_TYPE` is  `ONETRADING`, only funds available in fiat currencies will be seen for investment (required when `EXCHANGE_TYPE` is `ONETRADING` or `PAPER_TRADING`) | no       | None    |
...
| `OVERBOUGHT_THRESHOLD`        | Specify the RSI level to define overbought zone (from value to 100)                                                                                                                                        | no       | 80      |
| `TEST_INIT_CAPITAL`           | In testing mode, the amount of capital the bot has to invest (required when `EXCHANGE_TYPE` is `ONETRADING` or `PAPER_TRADING`).                                                                           | no       | None    |
