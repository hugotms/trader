...
```bash
docker build -t trader:3.0.2 https://raw.githubusercontent.com/hugotms/trader/v3.0.2/Dockerfile
```
...
```bash
docker run -d --name trader -e EXCHANGE_API_KEY=token -e MONGO_DB_HOST=hostname -e MONGO_DB_PASSWORD=secure trader:3.0.2
```
