...
```bash
docker build -t trader:3.1.0 https://raw.githubusercontent.com/hugotms/trader/v3.1.0/Dockerfile
```
...
```bash
docker run -d --name trader --restart unless-stopped -e EXCHANGE_API_KEY=token -e MONGO_DB_HOST=hostname -e MONGO_DB_PASSWORD=secure trader:3.1.0
```
