...
| `WINDOW_SIZE_FMA`          | Specify the number of units looked at to calculate fast exponential moving average (must be lower than `WINDOW_SIZE_MMA`)       | no      | 5     |
| `WINDOW_SIZE_MMA`          | Specify the number of units looked at to calculate middle exponential moving average (must be lower than `WINDOW_SIZE_SMA`)      | no      | 25     |
